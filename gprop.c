...
void calc_mom_space_scalarD(struct gluon_field *gf, double complex D[], double complex D4[], int MU_START, double XI){
            ...
            for(int qy=0; qy<Ns/2; qy++){
                for(int qz=0; qz<Ns/2; qz++){
                    ...
                    for(int mu=MU_START; mu<Nd; mu++){
                        ...
                        for(int i=0; i<8; i++){
                            if(mu == 0){
                                D[counter] += XI*XI*A_components[i]*A_neg_components[i];
                            }else{
